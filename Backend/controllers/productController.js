...
export const createProduct = async (req, res) => {
  try {
    const { brand, name, type_id, price } = req.body;
    if (!name || !type_id  || !brand || !price || typeof name !== 'string' || typeof brand !== 'string'  || typeof type_id !== 'number' || typeof price !== 'number')
      return res.status(400).json({ message: "brand, product name, type_id, price are required" });

    const [result] = await db.query(
      "INSERT INTO products (brand, type_id, name, price) VALUES (?, ?, ?, ?)",
      [brand, type_id, name, price || null]
    );
...
export const updateProduct = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      "UPDATE products SET name = ?, brand = ?, type_id = ?, price = ? WHERE id = ?",
      [name, brand, type_id, price, id]
    );

    res.status(200).json({ id, brand, name, type_id, price });
  } catch (err) {
    console.error(err);
